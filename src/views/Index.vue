  ...
  <div class="index-container">
    ...
    <el-card class="record-list-card" title="今日记录">
      <el-table :data="todayRecord" stripe>
        <el-table-column prop="record_type" label="记录类型" width="120"></el-table-column>
        <el-table-column prop="sub_type" label="详情" width="120"></el-table-column>
        <el-table-column prop="carbon_output" label="碳排放(kg)" width="120"></el-table-column>
        <el-table-column prop="carbon_reduce" label="减碳量(kg)" width="120"></el-table-column>
        <el-table-column prop="point" label="获得积分" width="100"></el-table-column>
...
const carbonOutput = computed(() => {
  ...
  const selectOption = document.querySelector(`el-option[value="${trafficForm.type}"]`)
  const coefficient = selectOption ? selectOption.getAttribute('coefficient') : 0
...
const addPoint = computed(() => {
  if (!trafficForm.mileage) return 0
  const selectOption = document.querySelector(`el-option[value="${trafficForm.type}"]`)
  return selectOption ? selectOption.getAttribute('point') : 0
})

// 提交出行记录
const submitTraffic = () => {
  const selectOption = document.querySelector(`el-option[value="${trafficForm.type}"]`)
  const subType = selectOption.textContent
